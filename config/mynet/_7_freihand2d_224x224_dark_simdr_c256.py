...
cfg = dict(
    ...
    MODEL=dict(
        name='mynet',
        input_channel=256,  # num_joints + 3 region map
        output_channel=21,  # num_joints + 3 region map
        num_stage=4,
        num_block=[2, 2, 2],
        pred_bbox=False,           # 模型是否预测边界框, 是则不进行旋转变换
        output_activation=False,   # 输出层是否添加swish激活函数,不能加ReLU会导致模型退化
        ),
    ...
    # 'Adam', 'SGD'
    OPTIMIZER=dict(type='Adam', lr=5e-4, warmup_steps=100, step_epoch=[60, 80]),
