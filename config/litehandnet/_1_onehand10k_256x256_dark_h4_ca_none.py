...
cfg = dict(
    ...
    # 'Adam', 'SGD'
    # OPTIMIZER=dict(type='SGD', lr=1e-2, warmup_steps=100),
    OPTIMIZER=dict(type='Adam', lr=5e-4, warmup_steps=10, step_epoch=[170, 200]),

    LOSS=dict(
        type='TopdownHeatmapLoss',
        loss_weight=[0.5, 1.],   # 四个输出的权重
        auto_weight=False,
    )
)
