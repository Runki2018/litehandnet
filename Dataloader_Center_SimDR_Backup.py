...
class CS_HandData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and self.is_augmentation:
            ...
            # crop_size = self.image_size[0] / self.cycle_detection_reduction, \
            #             self.image_size[1] / self.cycle_detection_reduction
            # img_crops, bbox, keypoints, is_crop = central_crop(image, keypoints, crop_size, prob=0.5)      
            
            # image = img_crops[0]  # TODO: 单手数据集特殊处理
    ...
    def generate_cd_gt(self, img, gt_kpts, bbox, target_weight=None):
        ...
        for i in range(batch):
            ...
            if w > h:
                nw = size_w
                factor = nw / w
                nh = min(int(h * factor + 0.5), size_h)
                nh = nh if nh % 2 == 0 else nh - 1
                pad = (size_h - nh) // 2 # 偶数必定整除
                pads = (0, 0, pad, pad)
            else:
                nh = size_h
                factor = nh / h
                nw = min(int(w * factor + 0.5), size_w)
                nw = nw if nw % 2 == 0 else nw - 1  # 确保偶数
                pad = (size_w - nw) // 2 # 偶数必定整除
                pads = (pad, pad)
                
            bbox_crop = np.array([[size_w // 2, size_h // 2, nw, nh]])
            
            # mode 默认为nearest， 其他modes: linear | bilinear | bicubic | trilinear
            img_crop = torch.nn.functional.interpolate(img_crop, size=(nh, nw)) 
            ...
            gt_kpts[i, 0] *= torch.tensor([factor, factor, 1]).to(current_device)
            if w > h:
                gt_kpts[i, 0] += torch.tensor([0, pad, 0]).to(current_device)
            else:
                gt_kpts[i, 0] += torch.tensor([pad, 0, 0]).to(current_device)

            # 将等长宽比放大后的图像填补为指定宽高的图像.
            # TODO: value 0~1 之间取什么值好？为了与卷积层下采样padding一致，取0
            img_crop = torch.nn.functional.pad(img_crop,
                                               pads,
                                               mode = 'constant',
                                               value = 0)
