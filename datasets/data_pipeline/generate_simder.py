...
class GenerateSimDR:
    def __init__(self, sigma=2, k=2):
        self.sigma = sigma
        self.k = int(k)   # simdr_split_ratio
        self.with_simdr = k > 0 and not isinstance(sigma, (list, tuple))
    ...
    def _generate_sa_simdr(self, joints, target_weight, image_size):
        ...
        target_x = np.zeros((num_joints, int(image_size[0] * self.k)),
                            dtype=np.float32)
        target_y = np.zeros((num_joints, int(image_size[1] * self.k)),
                            dtype=np.float32)

        for joint_id in range(num_joints):
            if target_weight[joint_id][0] > 0:
