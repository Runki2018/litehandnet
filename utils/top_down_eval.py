...
def _calc_distances(preds, targets, mask, normalize):
    ...
    # print(f"{normalize[:, None, :].shape=}")
    # print(f"{((preds - targets) / normalize[:, None, :]).shape=}")
    # print(f"{_mask.shape=}")
...
def get_PCK(output, target, target_weight, bbox, thr=0.2):
    """

    :param output: Tensor([N, K, H, W])
    :param target: Tensor([N, K, H, W])
    :param target_weight: Tensor([N, K, 1])
    :param bbox: Tensor([N, 1, 4])
    :param thr: float
    :return:
    """
    mask = (target_weight[..., 0] > 0).numpy()
    bbox = bbox.numpy()
    bbox_max_size = np.max(bbox[..., 2:], axis=2)
    threshold_bbox = np.concatenate([bbox_max_size, bbox_max_size], axis=1)
    _, pck, _ = pose_pck_accuracy(output.detach().cpu().numpy(), target.numpy(), mask, thr=thr, normalize=threshold_bbox)
    return pck


def pose_pck_accuracy(output, target, mask, thr=0.2, normalize=None):
    ...
    if normalize is None:
        normalize = np.tile(np.array([[H, W]]), (N, 1))  # (N , 2)

    pred, _ = _get_max_preds(output)
    gt, _ = _get_max_preds(target)

    # print(" keypoints pck acc ".center(30, '-'))
    # print(f"{H=}\t{W=}")
    # print(f"{normalize=}")
    # print(f"{thr=}")
    # print("-" * 40)
