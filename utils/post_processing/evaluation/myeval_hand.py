...
class MYeval_hand:
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm',
                 hand_key='keypoints', score_key='score'):
        '''
        Freihand数据集标注文件只有keypoints,没有区分lefthand_kpts, righthand_kpts。
        ...
        self.hand_key = hand_key         # 'keypoints' | 'lefthand_kpts' | 'righthand_kpts'
        self.score_key = score_key       # 'score' | 'lefthand_score' | 'righthand_score'
    ...
    def computeOks(self, imgId, catId):
        ...
        for j, gt in enumerate(gts):
            ...
            for i, dt in enumerate(dts):
                # d = np.array(dt['keypoints'][91*3:112*3])
                d = np.array(dt[self.hand_key])
                xd = d[0::3]; yd = d[1::3]
                if k1>0:
