...
class TopDownDecoder:
    ...
    def decode(self, meta, model_output):
        ...
        hm_preds, preds, maxvals = keypoints_from_heatmaps(
        ...
        result = {}
        result['preds'] = all_preds   # 预测出的原图关键点坐标，用于计算评价指标
        result['hm_preds'] = np.concatenate([hm_preds[:, :, 0:2] * 4, maxvals], axis=2)   # 预测出的裁剪图关键点坐标，用于可视化, 4倍上采样
