...
def aug_block(in_channels, out_channels, kernel_size, dk,dv, Nh, shape):
    return nn.Sequential(
            AugmentedConv(in_channels=in_channels, out_channels=out_channels,
                          kernel_size=kernel_size, dk=dk,dv=dv, Nh=Nh,
...
class Dense(nn.Module):
    def __init__(self, in_channels, growth_rate, kernel_size, iteration, Nh=4, aug=True, shape = 224):
        ...
        for i in range(1, self.iteration):
            self.arb.append(ARB(in_channels + growth_rate*i, growth_rate,
                                kernel_size=kernel_size, aug=aug, dk=0.1,
                                dv=0.1, Nh=Nh, shape = shape))
...
class light_Model(nn.Module):
    """
    https://github.com/hanchenchen/Attention-A-Lightweight-2D-Hand-Pose-Estimation-Approach-Pytorch
    """
    def __init__(self, conf=None):
        super(light_Model, self).__init__()
        # global config
        # config = conf  # not used
        input_shape = 224
        input_features = 3
