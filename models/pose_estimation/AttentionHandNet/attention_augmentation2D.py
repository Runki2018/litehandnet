...
class AugmentedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dk, dv, Nh, shape=0, relative=False, stride=1):
        ...
        self.dk = int(dk*out_channels)
        self.dv = int(dv*out_channels)
        self.Nh = Nh
        self.shape = shape
        ...
        if self.relative:
            self.key_rel_w = nn.Parameter(torch.randn((2 * self.shape - 1, self.dk // self.Nh), requires_grad=True))
            self.key_rel_h = nn.Parameter(torch.randn((2 * self.shape - 1, self.dk // self.Nh), requires_grad=True))
