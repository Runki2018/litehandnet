import numpy as np
from config.config import config_dict, DATASET

image_size = config_dict["image_size"]
thr_list = DATASET["thr_list"]
sigma_list = config_dict["hm_sigma"]
...
def generate_multi_heatmaps(batch_joints, heatmap_size: int, joints_weight, bbox=None, heatmap_sigma=None, img_size=None):
    """
        用于生成一张图片上有多只手的真值热图
        我移除了joints_vis， 因为可见性一直为1
        :param batch_joints:  [batch, nof_joints, 2]
        :param heatmap_size:  int, such as 88
        :param heatmap_sigma:  default 3
        :param joints_weight: 关键点权重，决定一个关键点的重要性，如果不可见则为0
        :param bbox: [n_hand, 4], [lx, ly, w, h], 非空时，根据框的大小动态选择sigma
        :param img_size: (int or None) 输入图像的大小
        :return: target, target_weight(1: visible, 0: invisible)
    """
    n_hand = batch_joints.shape[0]  # 因为当前测试的是单手数据集，所以batch数，即手的数目
    n_joints = batch_joints.shape[1]
    ...
    # todo: 如果一张图片中有多只手的时候，每个关键点的热图应该怎么生成？
    #  这里想到两种方法:
    #  (1) 在一个热图上同时预测多个关键点，这属于自底向上的方法，需要区分每个点属于那只手
    #  (2) 分别生产每只手的关键点热图，相应关键点的热图相加，但这样的话，可能会导致真值点附件重合的点，得分提高。
    ...
    for idx in range(n_hand):
        ...
        if bbox is not None and heatmap_sigma is None:
            heatmap_sigma = get_sigma(bbox[idx])
...
def get_multi_regions(bbox, heatmap_size, heatmap_sigma=None, img_size=None):
    ...
    region_weights = np.ones((n_hand, 3, 1), dtype=np.float32)

    for i_hand in range(n_hand):
        c = bbox[i_hand, :2]
        s = bbox[i_hand, 2:]
        if heatmap_sigma is None:
            heatmap_sigma = get_sigma(bbox=bbox[i_hand])  # 根据框的大小动态获取sigma
    ...
    return multi_hand_regions, region_weights
...
def get_hw_region_map(c, s, heatmap_size, heatmap_sigma, img_size):
    ...
    # heatmap range
    hm_x = max(0, ul[0]), min(br[0], heatmap_size)  # 0~brx, ulx~br, ulx~w_hm
    hm_y = max(0, ul[1]), min(br[1], heatmap_size)
    wh_region_map[0, hm_y[0]:hm_y[1], hm_x[0]:hm_x[1]] = gamma_x
    wh_region_map[1, hm_y[0]:hm_y[1], hm_x[0]:hm_x[1]] = gamma_y
...
def get_sigma(bbox):
    """
        根据bbox_area/image_area的大小来动态选择sigma，即热图高斯有效区域的范围由bbox大小决定
    :param bbox: [lx, ly, w, h]
    :return: int sigma default 3
    """
    if isinstance(sigma_list, int):
        return sigma_list

    if len(sigma_list) == len(thr_list) + 1:
        bbox_area = bbox[2] * bbox[1]
        image_area = image_size[0] * image_size[1]
        radio = bbox_area / image_area
        for i, thr in enumerate(thr_list):
            if radio <= thr:
                return sigma_list[i]
        return sigma_list[-1]
    elif isinstance(sigma_list, (int, tuple)):
        return sigma_list[0]
    else:
        raise ValueError(f"{len(sigma_list)=} vs {(len(thr_list) + 1) =}")


if __name__ == '__main__':
    # todo 验证 combine_together的算法正确性， 已经验证过没问题，可以正常运算
    a = np.zeros((3, 2, 5, 5))
    a[0, :, 2:, 2:] = np.random.randint(1, 10, (2, 3, 3)).astype(np.float32)
    a[1, :, 0:3, 0:3] = np.random.randint(1, 10, (2, 3, 3)).astype(np.float32)
    a[2, :, 1:, 1:] = np.random.randint(1, 10, (2, 4, 4)).astype(np.float32)

    print(a)
    b = combine_together(a)
    print(b)
