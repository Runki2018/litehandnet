    ...
    def test(self):
        ...
        if self.rank == 0:
            with torch.no_grad():
                xy_pck, xy_b_pck = [], []
                ap50_list, ap_list  = [], []
                hm_pck = [0] * self.n_out  # 用热图作比较计算的PCK
                kpt_pck = 0  # 用坐标作比较计算的PCK， 增加了关键点偏移的后处理过程。
                ...
                for meta in tqdm(self.test_loader, desc="testing "):
                    ...
                    avg_xy_pck = self.result_parser.evaluate_pck(vector2kpt, gt_kpts, thr=pck_thr)
                    avg_xy_b_pck = self.result_parser.evaluate_pck(vb2kpt, gt_kpts, thr=pck_thr)
                    ...
                    kpt_pck = kpt_pck + self.result_parser.evaluate_pck(hm2kpt, gt_kpts, thr=pck_thr) * img.shape[0]
                ...
                kpt_pck = kpt_pck / self.test_set.__len__() * 100
                ...
                # 记录训练数据
                self.save_dict["mPCK"].append(kpt_pck)
                ...
                if len(self.save_dict["mPCK"]) > 3:
                    ...
                    print("pck = {:.3f}".format(kpt_pck))

                if kpt_pck > self.best_mPCK:
                    self.best_mPCK = kpt_pck
